...
def log(
    note: str,
    # typer.Option(None...) makes --project an optional named flag rather than a positional argument; 
    ...
    entry_type: str = typer.Option("note", "--type", help="note/decision/bug/milestone/learning/question"),
) -> None:
    ...
    try:
        ...
        # httpx.ConnectError specifically means "couldn't even reach the server" (backend not running)
        typer.secho(
    ...
    typer.secho(f"Logged entry #{entry['id']} to project '{resolved_project}'", fg=typer.colors.GREEN)
